...
namespace AkberAli
{
    public partial class frmSaleReturn : Form
    {
        ...
        private void txtPieces2_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtPieces2.Text != "" && cmbProductName2.Text != "Please Select" && cmbCategory2.Text != "Please Select")
                {
                    ...
                    txtCartons2.Text = (Convert.ToDouble(txtPieces2.Text) / Convert.ToDouble(txtPcsInCN1.Text)).ToString();
            ...
            try
            {
                if (txtBookerID.Text != "" && txtAreaID.Text != "")
                {
                    if (txtTotalPrice1.Text != "0" && txtTotalPrice1.Text != "")
                    {
                        ...
                        String InsertQuery = "Insert into SalesReturn (Booker_ID, Area_ID, Shop_ID, Category_ID, Product_ID, Qty_Pcs, Cartons, Discount, Total_Amount, Date) values (" + txtBookerID.Text + "," +
                                                                                                                                                                              txtAreaID.Text + "," +
                                                                                                                                                                              txtShopID.Text + "," +
                    ...
                    MessageBox.Show("Sales Return has successfully saved.");
                    refresh();
                    SalesReturnView();
                }
                else
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("select SaleReturn_ID, b.Name as Booker, a.Area_Name as Area, s.Name as Shop, c.Category_Name as Category, " +
                                                    "p.Product_Name as Product, Qty_PCS, Cartons, Discount, Total_Amount, Date " +
                                                    "from SalesReturn sr " +
                                                    "join Booker b on b.Booker_ID = sr.Booker_ID " +
                                                    "join Area a on a.Area_ID = sr.Area_ID " +
                                                    "join Shops s on s.Shop_ID = sr.Shop_ID " +
                                                    "join Category c on c.Category_ID = sr.Category_ID " +
        ...
        private void gvSalesReturn_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ID = gvSalesReturn.Rows[e.RowIndex].Cells[0].Value.ToString();
            ...
            try
            {
                if (ID != null)
                {
                    ...
                    String updateQuery = "update inventory " +
                                          "set Quantity = Quantity - (select Qty_Pcs from SalesReturn where SaleReturn_ID = " + ID + ")" +
                                          "where Product_ID = (select Product_ID from SalesReturn where SaleReturn_ID = " + ID + ")";
                    ...
                    String DeleteQuery = "Delete from SalesReturn where SaleReturn_ID = " + ID;
                    SqlCommand cmdDelete = new SqlCommand(DeleteQuery, con);
                    con.Open();
                    cmdDelete.ExecuteNonQuery();
                    con.Close();
                    SalesReturnView();
                    MessageBox.Show("Sale return has successfully removed.");
                    ID = "";
                }
                else
                {
        ...
        private void stmPurchasesReturn_Click(object sender, EventArgs e)
        {

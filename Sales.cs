...
namespace AkberAli
{
    public partial class frmSales : Form
    {
        ...
        private void txtPieces13_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (cmbCategory13.Text == "Please Select" && txtPieces13.Text != "")
                {
                    MessageBox.Show("Please select category.");
                    cmbCategory13.Focus();
                    txtPieces13.Text = "";
                }
                if (cmbProductName13.Text == "Please Select" && cmbCategory13.Text != "Please Select" && txtPieces13.Text != "")
                {
                    MessageBox.Show("Please select product.");
                    cmbProductName13.Focus();
                    txtPieces13.Text = "";
                }
                if (txtPieces13.Text != "" && cmbProductName13.Text != "Please Select" && cmbCategory13.Text != "Please Select")
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        string qty = dr["Quantity"].ToString();
                        if (Convert.ToInt32(qty) <= Convert.ToInt32(txtPieces13.Text) && txtPieces13.Text != "0")
                        {
                        ...
                        if (Convert.ToInt32(qty) >= Convert.ToInt32(txtPieces13.Text) && txtPieces13.Text != "0")
                        {
                            txtCartons13.Text = (Convert.ToDouble(txtPieces13.Text) / Convert.ToDouble(txtPieces13.Text)).ToString();
                            txtTotalPrice13.Text = (Convert.ToDouble(txtPieces13.Text) * Convert.ToDouble(txtCostPrice13.Text)).ToString();
                        }
                    }
                    con.Close();
                }
                if (txtPieces13.Text == "" || txtPieces13.Text == "0")
                {
                    txtCartons13.Text = "";
                    txtTotalPrice13.Text = "";
                }
        ...
        private void txtPieces13_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsControl(e.KeyChar) && !char.IsDigit(e.KeyChar))
            {
                e.Handled = true;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtBookerID.Text != "" && txtAreaID.Text != "")
                {
                    ...
                    MessageBox.Show("Sales have successfully saved.");
                    refresh();
                    SalesView();
                }
                else
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtTotalPrice13.Text != "" && txtTotalPrice13.Text != "" && ID != null)
                {
                    ...
                    {
                        String updateQuery = "update inventory " +
                                              "set Quantity = Quantity + (select Qty_Pcs from Sales where Sale_ID = " + ID + ") - " + Convert.ToInt32(txtPieces13.Text) +
                                              " where Product_ID = (select Product_ID from Sales where Sale_ID = " + ID + ")";

                        String updateSaleQuery = "update sales " +
                                                   "set Category_ID = " + cmbCategory13.SelectedValue + ", Product_ID =  " + txtProductCode13.Text + ", Qty_Pcs =  " + txtPieces13.Text + ", Cartons = "
                    ...
                    else if (Product != txtProductCode13.Text)
                    {
                        String updateQuery1 = "update inventory " +
                                              "set Quantity = Quantity + (select Qty_Pcs from Sales where Sale_ID = " + ID + ")" +
                                              " where Product_ID = (select Product_ID from Sales where Sale_ID = " + ID + ")";
                        String updateQuery2 = "update Inventory set Quantity = Quantity - " + Convert.ToInt32(txtPieces13.Text) + " where Product_ID = " + txtProductCode13.Text;
                        String updateSaleQuery = "update sales " +
                                                   "set Category_ID = " + cmbCategory13.SelectedValue + ", Product_ID =  " + txtProductCode13.Text + ", Qty_Pcs =  " + txtPieces13.Text + ", Cartons = "
                                                   + txtCartons13.Text + " , Total_Amount = " + Convert.ToDouble(txtTotalPrice13.Text) + " - " + (Convert.ToDouble(txtTotalPrice13.Text) * Convert.ToDouble(discount) / 100)
                                                   + " where Sale_ID = " + ID;
